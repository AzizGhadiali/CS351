/*
  ...
  A program to test the polygon function
*/
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) {
  ...
  printf("polygon init and set\n");
  polygon_init( &thing1 );
  polygon_set( &thing1, 20, &( p[42] ) );

  printf("polygon create\n");
  thing2 = polygon_createp( 10, &(p[105] ) );
  ...
  polygon_drawFill( &thing1, src, Blue );
  polygon_drawFill( thing2, src, Red );
  ...
  polygon_set( thing2, 5, &(p[500]) );
  polygon_drawFill( thing2, src, Green );
