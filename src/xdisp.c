/*
  ...
  package.
	...
	Modified by Aziz Ghadiali Spring 2018
...
void rotateVRC(View3D *view, Vector *u, float angleVUP, float angleU){
	...
	matrix_translate(&t1, -(view->vrp.val[0] + vpnN.val[0] * 0.5), 
							-(view->vrp.val[1] + vpnN.val[1]* 0.5), 
							-(view->vrp.val[2] + vpnN.val[2]* 0.5));
	...
	Matrix t2;
	matrix_identity(&t2);
	matrix_translate(&t2, (view->vrp.val[0] + vpnN.val[0] * 0.5), 
							(view->vrp.val[1] + vpnN.val[1]* 0.5), 
							(view->vrp.val[2] + vpnN.val[2]* 0.5));
	...
	matrix_rotateXYZ(&tvrc, &view->vpn, u, &view -> vup); //check
	
	tvrc.mat[0][3] = 1;
	tvrc.mat[3][3] = 0;
	for(int i = 0; i < 3; i++){
		tvrc.mat[3][i] = vpnN.val[i];
	}
	...
	*/
	...
	point_set3D(&view -> vrp, result.mat[0][0], result.mat[0][1], result.mat[0][2]);	
	...
	*/
	...
	vector_set(&view -> vup, result.mat[2][0], result.mat[2][1], result.mat[2][2]); 
	...
	*/
	vector_set(&view -> vpn, result.mat[3][0], result.mat[3][1], result.mat[3][2]); 	
...
int main(int argc, char *argv[])
{
  ...
  *ximp = NULL;
	...
	float roty = 0.0;
			...
			matrix_identity( &VTM );
			matrix_identity( &GTM );
			point_set3D( &(view.vrp),  3 + xoff,  3.0 + yoff, 3 + zoff );
			vector_set(  &(view.vpn), -3.0, -3.0,  -3.0);
			...
			// module_scale( cube, 3, 1, 3 );
			// module_cube(cube, 1);
			
			
			// for (i=0;i<8;i++){
				module_color(cube, &c1);
				module_cube(cube, 1);

				module_color(cube, &c2);
				module_translate(cube, 1, -1, 0);
				module_cube(cube, 1);

				module_translate(cube, -1, 0, 1);
				module_color(cube, &c3);
				module_cube(cube, 1);

				module_translate(cube, -1, 0, 1);
				module_color(cube, &c4);
				module_cube(cube, 1);

				module_translate(cube, -1, 0, 1);
				module_color(cube, &c5);
				module_cube(cube, 1);

				module_translate(cube, 0, 1, -1);
				module_color(cube, &c6);
				module_cube(cube, 1);

				module_translate(cube, 1, 0, -1);
				module_color(cube, &c7);
				module_cube(cube, 1);

				module_translate(cube, 0, 1, -1);
				module_color(cube, &c8);
				module_cube(cube, 1);

				module_translate(cube, -1, 0, 1);
				module_color(cube, &c9);
				module_cube(cube, 1);

				module_translate(cube, 0, 1, -1);
				module_color(cube, &c10);
				module_cube(cube, 1);
  ...
  while(!done) {
		...
		{ // replace this block of code with your own, or use it to test your cube
			...
			point_set3D( &(view.vrp),  2 + xoff * u.val[0] + yoff * view.vup.val[0] + zoff * view.vpn.val[0]
									,   0.5 + xoff * u.val[1] + yoff * view.vup.val[1] + zoff * view.vpn.val[1]
									,	0 + xoff * u.val[2] + yoff * view.vup.val[2] + zoff * view.vpn.val[2]);
