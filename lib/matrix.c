/*
  ...
  matrix.c
  March 15, 2018
  contains all functions for matrices
...
void matrix_xformPolygon(Matrix *m, Polygon *p){
	...
	for (int x = 0; x<p->nVertex; x++){
		...
		matrix_xformVector(m, &p->normal[x], &temp1);
		p->vertex[x].val[0] = temp.val[0];
		p->vertex[x].val[1] = temp.val[1];
		p->vertex[x].val[2] = temp.val[2];
		p->vertex[x].val[3] = temp.val[3];

		p->normal[x].val[0] = temp1.val[0];
		p->normal[x].val[1] = temp1.val[1];
		p->normal[x].val[2] = temp1.val[2];
		p->normal[x].val[3] = temp1.val[3];
	}
}
...
void matrix_rotateZ(Matrix *m, double cth, double sth){
	Matrix rotateZ;
	// printf("Rotate Z before clear");
// 	matrix_print(m, stdout);
	matrix_identity(&rotateZ);
//  	printf("Rotate Z after initialize");
//  	matrix_print(&rotateZ, stdout);
	rotateZ.mat[0][0] = cth;
	rotateZ.mat[0][1] = -sth;
	rotateZ.mat[1][0] = sth;
	rotateZ.mat[1][1] = cth;
	
// 	printf("Rotate Z after putting in matrix values");
//   	matrix_print(&rotateZ, stdout);
	matrix_multiply(&rotateZ, m, m);
// 	printf("Rotate Z after multiplying matrices");
// 	matrix_print(&rotateZ, stdout);
// 	printf("M after multiplying matrices");
// 	matrix_print(m, stdout);
